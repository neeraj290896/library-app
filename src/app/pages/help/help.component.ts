...
import { Guidance } from '@app/shared/models/api.models';
import { GuidanceService } from '@app/shared/services/guidance.service';
...
export class HelpComponent {
  ...
  loadGuidanceDetails():void{
     this.guidanceService.getGuidanceDetails().subscribe({
                next: (data: Guidance[]) => {
                ...
                error: (err) => {
                    console.error('Error loading guidance details:', err);
                }
            });
  }


  toggleFAQ(faq: Guidance): void {
