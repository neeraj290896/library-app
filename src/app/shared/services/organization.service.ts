import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { OrganizationDetails, DepartmentDetails } from '../models/api.models';
import { environment } from '../../../environments/environment';

@Injectable({
    providedIn: 'root'
})
export class OrganizationService {
    private readonly baseUrl = `${environment.apiUrl}/api/Organization`;

    constructor(private http: HttpClient) { }

    getOrganizationDetails(): Observable<any> {
        return this.http.get(`${this.baseUrl}/GetOrganizationDetails`);
    }

    addOrganizationDetails(payload: OrganizationDetails): Observable<any> {
        return this.http.post(`${this.baseUrl}/AddOrganizationDetails`, payload);
    }

    updateOrganizationDetails(payload: OrganizationDetails): Observable<any> {
        return this.http.post(`${this.baseUrl}/UpdateOrganizationDetails`, payload);
    }

    deleteOrganizationDetails(payload: OrganizationDetails): Observable<any> {
        return this.http.post(`${this.baseUrl}/DeleteOrganizationDetails`, payload);
    }

    getDepartmentDetails(): Observable<any> {
        return this.http.get(`${this.baseUrl}/GetDepartmentDetails`);
    }

    addDepartmentDetails(payload: DepartmentDetails[]): Observable<any> {
        return this.http.post(`${this.baseUrl}/AddDepartmentDetails`, payload);
    }

    updateDepartmentDetails(payload: DepartmentDetails[]): Observable<any> {
        return this.http.post(`${this.baseUrl}/UpdateDepartmentDetails`, payload);
    }

    deleteDepartmentDetails(payload: DepartmentDetails[]): Observable<any> {
        return this.http.post(`${this.baseUrl}/DeleteDepartmentDetails`, payload);
    }

    uploadOrganizationImage(payload: FormData): Observable<any> {
        return this.http.post(`${this.baseUrl}/UploadOrganizationImage`, payload);
    }
}
