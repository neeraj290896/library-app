...
export class BookService {
    ...
    getISBNDetails(_isbn: string): Observable<any> {
        const url = `https://openlibrary.org/api/books?bibkeys=ISBN:${encodeURIComponent(_isbn)}&format=json&jscmd=data`;

        console.log('Fetching ISBN details from URL:', url);

        return this.http.get<any>(url);
    }

    getAdvancedSearchForISBNDetails(_isbn: string): Observable<any> {
        
        const url = `https://www.googleapis.com/books/v1/volumes?q=isbn:${encodeURIComponent(_isbn)}&key=${environment.googleBooksApiKey}`;
